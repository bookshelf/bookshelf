...
function Bookshelf(knex) {
  ...
  function resolveModel(input) {
    ...
    return (
      ...
      (function() {
        throw new errors.ModelNotResolvedError(`The model ${input} could not be resolved from the registry plugin.`);
      })()
    );
  }
  ...
  const bookshelf = {
    ...
    model(name, Model, staticProps) {
      if (Model) {
        preventOverwrite(this.registry.models, name);
        if (_.isPlainObject(Model)) Model = this.Model.extend(Model, staticProps);
