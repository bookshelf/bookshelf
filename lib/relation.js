...
_.extend(BookshelfRelation.prototype, {
  ...
  through: function(source, Target, options) {
    ...
    var throughTableName = _.result(Target.prototype, 'tableName');
    var throughIdAttribute = _.result(Target.prototype, 'idAttribute');
    
    this.throughTargets.push({
      target: Target,
      tableName: throughTableName,
      idAttribute: throughIdAttribute,
      foreignKey: options.throughForeignKey || singularMemo(throughTableName) + '_' + throughIdAttribute,
  ...
  key: function(keyName) {
    ...
    if (keyName === 'throughForeignKey') {
      return this[keyName] = singularMemo(_.last(this.throughTargets).tableName) + '_' + _.last(this.throughTargets).idAttribute;
    }
  ...
  joinColumns: function(knex) {
    ...
    if (this.isThrough()) {
      columns.push(_.last(this.throughTargets).idAttribute);
      if (this.throughTargets.length > 1)
        columns.push(this.throughTargets[this.throughTargets.length - 2].foreignKey);
      else
  ...
  joinClauses: function(knex) {
    ...
    if (this.type === 'belongsTo' || this.type === 'belongsToMany') {
      ...
      if (!_.isEmpty(this.throughTargets)) {

        var scope = this;
        _.each(this.throughTargets, function(through, i) {
          
          knex.join(
            through.tableName,
            through.tableName + '.' + (i == 0 ? targetKey : scope.throughTargets[i - 1].foreignKey), '=',
            (i === 0 ? scope.targetTableName : scope.throughTargets[i - 1].tableName) + '.' + scope.targetIdAttribute
          );
        });
        
        // A `belongsTo` -> `through` is currently the only relation with two or more joins.
        if (this.type === 'belongsTo') {

          knex.join(
           this.parentTableName,
           _.last(this.throughTargets).tableName  + '.' + _.last(this.throughTargets).idAttribute, '=',
           this.parentTableName + '.' +  singularMemo(_.last(this.throughTargets).tableName) + '_' + _.last(this.throughTargets).idAttribute
          );
        }
        ...
        if (this.type === 'belongsTo') {
          knex.join(
            ...
            joinTable + '.' + _.last(this.throughTargets).idAttribute, '=',
  ...
  joinTable: function() {
    if (this.isThrough()) return _.last(this.throughTargets).tableName
  ...
  isThrough: function() {
    return !_.isEmpty(this.throughTargets);
  },
  isJoined: function() {
