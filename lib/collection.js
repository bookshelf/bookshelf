...
define(function(require, exports, module) {
  ...
  // Circular dependencies exist between the Eager, Relation, and Model
  // objects, so we'll just keep around a single letter variable referencing
  // their `exports` object, and then get the actual constructor when needed.
  var E = require('./eager');
  var R = require('./relation');
  var M = require('./model');
  ...
  _.extend(Collection.prototype, _.omit(Backbone.Collection.prototype, 'model'), Events, {
    ...
    fetch: function(options) {
      ...
      return this.sync(options)
        ...
        .then(function(response) {
          ...
          return new E.Eager(collection.models, response, new collection.model())
            .fetch(options)
    ...
    load: function(relations, options) {
      ...
      return new E.Eager(this.models, this.toJSON(options), new this.model())
        .fetch(options)
    ...
    create: function(model, options) {
      ...
      var collection  = this;
      var relatedData = this.relatedData || new R.Relation();
    ...
    _prepareModel: function(attrs, options) {
      if (attrs instanceof M.Model) return attrs;
