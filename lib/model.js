...
const BookshelfModel = ModelBase.extend(
  {
     ...
     * @param {Model} Target Constructor of {@link Model} targeted by the join.
     * @param {string} [foreignKey]
    ...
    morphTo(morphName) {
      ...
      return this._relation('morphTo', null, {
        morphName,
        columnNames,
        candidates
      }).init(this);
    ...
    /**
     * Helps to create dynamic relations between {@link Model models} where a
     * {@link Model#hasOne hasOne} or {@link Model#belongsTo belongsTo} relation may run through
     * another `Interim` model. This is exactly like the equivalent
     * {@link Collection#through collection method} except that it applies to the models that the
     * above mentioned relation methods return instead of collections.
     *
     * This method creates a pivot model, which it assigns to {@link Model#pivot model.pivot} after
     * it is created. When serializing the model with {@link Model#toJSON toJSON}, the pivot model
     * is flattened to values prefixed with `_pivot_`.
     *
     * A good example of where this would be useful is if a paragraph {@link Model#hasMany belongTo}
     * a book *through* a chapter. See the example above on how this can be expressed.
     *
