 ...
 * @description
 *
 * Returns the value of an attribute like it was imediatelly **after** the last
 * {@link Model#fetch fetch} or {@link Model#save save}. This is useful for
 * getting back the original attribute value after it's been changed with
 * {@link Model#set set} and before those changes are saved to the database.
 *
 ...
 *
 * Note that this will not return the value of an attribute *before* the last
 * `save`. Additionally it will return the current value if an attribute hasn't
 * changed.
 *
 * @example
 * Author.forge({id: 1}).fetch().then(function(author) {
 ...
 * @description
 *
 * Returns a copy of the {@link Model model}'s attributes like they were
 * imediatelly after the last {@link Model#fetch fetch} or
 * {@link Model#save save}. This is mostly useful for getting a diff of the
 * model's attributes after changing some of them and before those changes are
 ...
 *
 * Note that this will return the exact same values as the current attributes
 * if no changes have been made to the model since the last `save` or `fetch`.
 *
 * @example
 * Author.forge({id: 1}).fetch().then(function(author) {
 *   author.get('name') // Alice
